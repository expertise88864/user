...
class DeliveryTests(unittest.TestCase):
    def test_hyperlink_failures_are_not_masked(self):
        source = (d.ROOT / '.github/workflows/hyperlink.yml').read_text(encoding='utf-8')
        self.assertNotIn('||', source)
        self.assertNotIn('continue-on-error:', source)
        self.assertIn('run: python _check_hyperlink.py', source)
        import _check_hyperlink as h
        from types import SimpleNamespace
        with patch.object(h, 'stage'), patch.object(h.subprocess, 'run', return_value=SimpleNamespace(returncode=2)):
            self.assertEqual(h.main(), 2)

    def test_hyperlink_routes_keep_unknown_targets_and_fragments(self):
        import _check_hyperlink as h
        text = '<a href="/tools#absent-fixture">x</a><a href="/nonexistent-fixture#bad">y</a>'
        rendered = h.routed_html(text, d.ROOT / 'index.html')
        self.assertIn('/tools.html#absent-fixture', rendered)
        self.assertIn('/nonexistent-fixture#bad', rendered)
    ...
    def test_production_smoke_rejects_stale_assets_and_checks_all_routes(self):
        ...
        def response(request, **kwargs):
            data = b'<title>Site</title><main>Content</main>' if request.full_url.endswith('/') else b'current'
            result = Response(data)
            result.url = request.full_url
            return result
        api = SimpleNamespace(get=lambda _: {'commit': {'sha': SHA}})
        with patch.object(d, 'deployment_url', return_value='https://exact.vercel.app'), \
                patch.object(d.urllib.request, 'urlopen', side_effect=response), \
                patch.object(d.subprocess, 'check_output', return_value=b'current') as expected:
            self.assertEqual(len(d.production_smoke(api, SHA, cfg)['smoke_routes']), 5)
            expected.return_value = b'previous'
            with self.assertRaises(d.Blocked):
                d.production_smoke(api, SHA, cfg)
    ...
    def test_dispatch_requires_explicit_project_support(self):
        ...
        for changed in ({"head_branch": "codex/manual"}, {"actor": {"login": "expertise88864"}}):
            with self.assertRaises(d.Blocked):
                d.select_run([{**run, **changed}], "ci.yml", SHA, "candidate", True)
        failed_push = {**run, "event": "push", "conclusion": "failure", "id": 0}
        with self.assertRaises(d.Blocked):
            d.select_run([failed_push, run], "ci.yml", SHA, "candidate", True)

...
def main() -> int:
    ...
    require(errors, "blog/pagefind-search.js", "url.includes('\\\\')", "Pagefind result URLs should reject browser-normalized backslash authorities")
    require(errors, "blog/pagefind-search.js", "url[0] !== '/' || url[1] === '/'", "Pagefind result URLs should require a single-rooted same-site path")
    require(errors, "blog/pagefind-search.js", "requestId !== searchRequestId", "Pagefind should ignore stale asynchronous search responses")
    ...
    require(errors, "blog/blog-shared.js", "url[0] !== '/' || url[1] === '/'", "Command palette URLs should require a single-rooted same-site path")
    ...
    require(errors, "admin/session-auth.js", "!Number.isFinite(expiry) || expiry <= Date.now()", "Admin PAT expiry should fail closed")
    require(errors, "admin/session-auth.js", "setPat(token, true)", "Admin reload should restore the cookie session without extending PAT expiry")
    require(errors, "admin.html", "auth.restoreSession()", "Admin UI should restore its cookie through the shared controller")
    ...
    require(errors, "admin.html", "frame.setAttribute('sandbox', 'allow-same-origin');", "Admin preview iframe should block article scripts while remaining editable")
    require(errors, "admin.html", "d.querySelectorAll('script').forEach(s => s.remove());", "Admin preview should remove all repository-provided scripts")
    require(errors, "admin.html", "await auth.setPat(value);", "Admin login should mint the HttpOnly server session")
    require(errors, "admin.html", "PAT = auth ? auth.getPat() : '';", "Admin shell should use the shared expiring credential reader")
    require(errors, "admin/admin-extras.js", "headers: { 'Content-Type': 'application/json' },", "Admin same-origin writes should rely on the HttpOnly session")
    forbid(errors, "admin/admin-extras.js", "headers: { 'Content-Type': 'application/json', Authorization: 'token ' + getPat() }", "Admin should not resend PATs to cookie-authenticated same-origin APIs")
    ...
    require(errors, "admin/edit.html", "function sanitizeEditableHtml(html)", "Simple editor should sanitize article and draft HTML before DOM insertion")
    require(errors, "admin/edit.html", "script,iframe,object,embed,form,base,meta,link,style,template", "Simple editor should remove active and document-level elements")
    require(errors, "admin/edit.html", "name.indexOf('on') === 0", "Simple editor should strip inline event handlers")
    require(errors, "admin/edit.html", "name === 'srcdoc'", "Simple editor should strip iframe srcdoc payloads")
    require(errors, "admin/edit.html", "name === 'ping' || name === 'formaction'", "Simple editor should strip request-triggering link and form attributes")
    require(errors, "admin/edit.html", "!isSafeEditorUrl(attr.value, false)", "Simple editor should validate navigation URLs")
    require(errors, "admin/edit.html", "$editor.innerHTML = sanitizeEditableHtml(html);", "Simple editor should sanitize before rendering content")
    require(errors, "admin/edit.html", "exec('insertHTML', sanitizeEditableHtml(html));", "Simple editor should sanitize pasted and dropped HTML")
    require(errors, "admin/edit.html", "anchor.textContent = url;", "Simple editor should build inserted links without raw HTML interpolation")
    require(errors, "admin/edit.html", "parsed.protocol === 'http:'", "Simple editor URL checks should use a protocol allowlist")
    forbid(errors, "admin/edit.html", "$editor.innerHTML = html;", "Simple editor should not insert article or draft HTML directly")
    forbid(errors, "admin/edit.html", "var anchor = '<a href=\"' + url", "Simple editor should not assemble links from raw URL strings")

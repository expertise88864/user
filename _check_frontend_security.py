...
def main() -> int:
    ...
    require(errors, "admin/admin-extras.js", "!Number.isFinite(exp) || exp <= Date.now()", "Admin PAT expiry should fail closed")
    require(errors, "admin/admin-extras.js", "createServerSession(existingPat)", "Admin reload should restore the cookie session without extending PAT expiry")
    ...
    require(errors, "admin.html", "await window.cdAdminAuth.setPat(v);", "Admin login should mint the HttpOnly server session")
    require(errors, "admin.html", "!Number.isFinite(exp) || exp <= Date.now()", "Admin shell should reject expired or malformed PAT deadlines")

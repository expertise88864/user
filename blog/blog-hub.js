...
// dn-articles-desc:start
  DN.ARTICLES_DESC = {
    ...
    'prurigo-nodularis': {desc:'結節性癢疹（PN）為什麼越抓越硬、越硬越癢？依 TDA 2024 共識整理診斷工具（WI-NRS、IGA-PN）、第一線外用、Dupilumab、Nemolizumab（2024 FDA 核准）、JAK 抑制劑。皮膚科醫師整理。',desc_en:'Prurigo nodularis (PN) is a chronic, refractory, intensely pruritic skin disease characterized by symmetrically distributed, firm, well-demarcated nodules (0.5-3 cm)...'},
    ...
    'spironolactone-dermatology-safety': {desc:'2026 JAAD 兩篇真實世界研究整合：≥45 歲女性吃 spironolactone（愛達信錠 / 安達通® / Aldactone®）整體高血鉀 10.1%、≥65 歲 + 共病高達 28.1%；但 97.5% 屬輕度、62.5% 不影響處置。合併低劑量 minoxidil 同時起始反而少 64.8% 多毛。整理分層監測與台灣自費實況。',desc_en:'Two June-2026 JAAD studies from Mass General Brigham synthesized: Gregoire et al. (n=398) — in women ≥45 on spironolactone for dermatologic indications, overall...'},
    ...
    'toenail-mechanical-disorders': {desc:'腳趾甲變黃變厚不一定是灰指甲——Studies show that  9 成趾甲變形其實來自鞋子壓迫與腳型問題。本文整理 12 種常見趾甲變形（剝離、嵌甲、Beau’s line、夾擊甲、甲下繭等）、4 種足部結構異常（Morton 趾、拇指外翻、扁平足、開張足）、糖尿病趾甲照護與治療選擇，含台灣健保藥品與商品名。整合 2026 JAAD 最新 review。',desc_en:'Over 90% of toenail dystrophy is mechanical, not fungal. Patient-friendly guide to the 12 most common types (onycholysis, subungual heloma, Beau lines, retronychia...'},
    'topical-acids-patient': {desc:'外用酸類保養五大家族：A 酸（Tretinoin / Adapalene）、A 醇、杜鵑花酸、果酸 AHA、水楊酸 BHA — 適合誰、不能與誰併用、孕婦能否使用、會不會反黑爆痘？附入門劑量階梯與混搭禁忌表。',desc_en:'Pharmacies and beauty counters are full of ”acids” — some for acne, some for brightening, some for anti-aging. But ingredient labels (Retinol, Retinaldehyde...'},

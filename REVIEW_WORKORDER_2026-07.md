...
- [x] **Phase 1 醫療計算器 — 完成 2026-07-08(19/19 審完)**。**19 個計分公式本身全部正確**(範圍/係數/bands 對得上文獻)。缺陷在公式之外,全數記入 TECH_DEBT P0 段(TD-20~25),**皆未修**(🟡 待醫師/使用者定案):
  - 必查點 1 Norwood/Ludwig → **確認 P0 缺陷**:女性 Ludwig 被標 I–VII(應 I–III)+ 套用男性階段描述 → TD-20。
...
- [x] **Phase 4 驗證器可信度 + 部署基建 — 完成 2026-07-10**(假綠燈:**0 支**)。A. 抽查 7 支驗證器:`_check_robots.py` 是黃金標準(獨立硬編碼 REQUIRED_BLOCKED 防同源連坐)、`_audit_jsonld`/`_check_sitemap` 真結構/行為驗證;安全兩支(api/frontend)是字串契約 regression-lock、覆蓋比名稱窄(TD-31 🟡 建議補廣掃,未動工需 codex)。B. 基建:`sw.js`/`vercel.json`/8 CI workflows **無 P0/P1**——無 `pull_request_target`、actions 全 SHA-pin、scoped perms、`[skip actions]` 防迴圈、輸入驗證紮實、`indexnow` choice 白名單、cache-poisoning 已防護。**修復 TD-30**(`_check_sitemap.py` 補 `<lastmod>` W3C 格式驗證,封住 TD-06 溜進去的缺口,🟢)。新記 TD-31(安全 checker 覆蓋缺口,🟡 建議)、TD-32(P3 基建觀察)。全 gate 綠 + Codex(gpt-5.6-sol)APPROVE。HANDOFF:Phase 4 交付,下一個 = Phase 5(前端核心 runtime `blog-shared.js` 2004/`assets/inline`/`blog-hub.js`)。
- [x] **Phase 5 前端核心 runtime — 完成 2026-07-10**(Codex gpt-5.6-sol APPROVE)。深讀 `blog-shared.js`(2004)/`assets/inline/*`/`blog-hub.js`/`pagefind-search.js`。**無 P0/P1**:`?q=` 走 input.value(非 innerHTML)→ 安全;雙語 innerHTML author-trusted;無 bfcache 重掛;localStorage 多處已守衛;D-11 單一入口未繞過(analytics-loader 三重 gate);#14 source/min **全同步**(無 stale)。**修 TD-33**(blog-shared.js 三處硬化:detectLang localStorage 守衛、renderPagefind data() 後 freshness re-check #12、excerpt escapeHtml #9)+ **TD-34**(perf-budget checker 量到本機 CRLF 非部署 LF → 改量 LF-normalized;根因 `_minify` CRLF 寫入降 P3)。`_minify`+check-js+frontend-security+smoke+全 gate 綠。HANDOFF:下一個 = Phase 6(次要 bundles:`blog-diagrams`/`blog-article-visuals`/`blog-article-reading`/`blog-article-footer` + 總結報告)。
- [x] **Phase 6 次要 bundles + 總結報告 — 完成 2026-07-10**(Codex gpt-5.6-sol APPROVE)。深讀 `blog-diagrams`(1189)/`blog-article-visuals`(664)/`blog-article-reading`(604)/`blog-article-footer`(596)。**無 P0/P1**:diagrams 零危險 sink(純 SVG);visuals heroSvg/lightbox 安全;footer localStorage 全守衛、innerHTML author/static、mailto encoded。**修 TD-35**(reading.js:addFontSizer localStorage 補守衛 + TOC 對 H2 text/id escape,防「PASI < 10」類標題誤渲染)。全 gate 綠。**總結報告寫入 TECH_DEBT 末段「📊 全案總結」**:6 phases 全交付、全站體質優良、無未修 P0/P1;待決策僅 TD-05(使用者已暫緩)。**✅ REVIEW_WORKORDER 全案完成。**
